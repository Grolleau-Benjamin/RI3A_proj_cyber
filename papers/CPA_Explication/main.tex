...
\subsection{Modèle de fuite : poids de Hamming}

Pour relier la valeur intermédiaire AES aux traces de consommation, la CPA nécessite un modèle de fuite reflétant la consommation du microcontrôleur.
Le modèle retenu dans cette étude est le poids de Hamming de la sortie de la SBox, une approximation classique du nombre de bascules qui changent d’état lors de l’écriture d’une valeur dans un registre.

Le modèle de fuite est donc défini comme :
\begin{equation}
\mathrm{leak} = \mathrm{HW}(v)
\end{equation}
où la fonction poids de Hamming s’écrit :

\begin{equation}
\mathrm{HW}(x) = \sum_{i=0}^{7} ( (x \gg i) \& 1 )
\end{equation}

Cette expression compte simplement le nombre de bits à 1 dans l’octet $x$, ce qui fournit un modèle de consommation entre 0 et~8.

La figure~\ref{fig:hw_table} illustre le poids de Hamming pour toutes les valeurs possibles entre \texttt{0x00} et \texttt{0xFF}.

\begin{figure}[h]
\centering
\includegraphics[width=1\linewidth]{img/hw_table.png}
\caption{Poids de Hamming pour toutes les valeurs possibles (0x00 à 0xFF).}
\label{fig:hw_table}
\end{figure}
